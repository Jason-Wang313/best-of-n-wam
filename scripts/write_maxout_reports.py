...
- The strongest contact-rich external evidence is Gymnasium Robotics Fetch plus Meta-World ML1, RoboSuite Panda, ManiSkill state mode, RoboCasa three-task pick-place, broad four-task atomic-manipulation, and 12-task family artifacts, and LIBERO Spatial rollout-pool validation, but still not real hardware.
...
- Gymnasium/MuJoCo, Gymnasium Robotics Fetch, Meta-World ML1, RoboSuite Panda, RGB WAM-lite on Reacher-v5 and Fetch frames, ManiSkill3 state-mode, RoboCasa three-task pick-place-family plus broad four-task and 12-task kitchen learned-WAM artifacts, and LIBERO Spatial three-task rollout-pool learned-WAM artifacts validate the benchmark path without claiming hardware evidence.
