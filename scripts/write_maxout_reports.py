...
Benchmark-full plus Fetch, Meta-World, RoboSuite, ManiSkill state-mode, RoboCasa single-task, three-task pick-place-family, broad four-task, 12-task, 24-task family, extra four-task, and combined 28-task learned WAM-lite, LIBERO Spatial three-task rollout-pool WAM-lite, LIBERO Object sparse-success scripted smoke, LIBERO learned action-head smoke, LIBERO time-conditioned autonomous low-dimensional BC smoke, LIBERO RGB/proprio/language BC smoke, visual-, blocker-probe-, and audit-validated: learned-toy, multi-env toy validation, Gymnasium/MuJoCo Reacher-v5 benchmark validation, Gymnasium Robotics Fetch validation, Meta-World ML1 manipulation validation, RoboSuite Panda manipulation validation, ManiSkill3 state-mode manipulation validation, RoboCasa kitchen smoke plus single-task, three-task, broad task family, 12-task, 24-task family, extra four-task, and combined 28-task learned-WAM validation, LIBERO rollout-pool learned-WAM validation, LIBERO scripted sparse-success smoke, LIBERO learned action-head sparse-success smoke, LIBERO time-conditioned low-dimensional BC sparse-success smoke, LIBERO RGB/proprio/language sparse-success smoke, toy visual mode, Reacher RGB WAM-lite, Fetch RGB WAM-lite, ManiSkill visual/EE-control blocker probing, and inference-value audit framework artifacts.
...
- Full RoboCasa-wide learned-WAM rollout collection beyond the current pick-place-family, broad atomic-manipulation, 12-task, 24-task family, extra four-task, and combined 28-task artifacts.
...
- `python -m pytest -q`: passed with `40 passed`.
