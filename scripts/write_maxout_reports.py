...
- No real robot, DreamZero, UWM, modern VLA, or full LIBERO policy result is claimed; RoboCasa is verified for pick-place-family, broad atomic kitchen, and 12-task family rollout-pool artifacts, and LIBERO is verified as a three-task rollout-pool dense-utility artifact plus narrow scripted, learned action-head, time-conditioned low-dimensional BC, and RGB/proprio/language BC sparse-success smokes.
...
- LIBERO RGB/proprio/language BC smoke: feature-kNN behavior cloning trained on `{libero_visual_language_bc.get('train_examples')}` scripted action examples and evaluated on `{libero_visual_language_bc.get('eval_episodes')}` heldout sparse-success episodes over the scripted-success Object subset without object state, task IDs, phase labels, or target-point commands at evaluation time.
