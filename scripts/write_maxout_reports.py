...
- RoboCasa registry coverage audit: registered task IDs `{robocasa_catalog.get('registry_count')}`; verified-artifact task IDs `{robocasa_catalog.get('verified_artifact_task_count')}`; coverage fraction `{fmt(robocasa_catalog.get('coverage_fraction'))}`.
...
- RoboCasa catalog coverage audit: local registry contains `{robocasa_catalog.get('registry_count')}` task IDs; verified rollout-pool artifacts cover `{robocasa_catalog.get('verified_artifact_task_count')}` of them. This is coverage accounting, not validation for uncovered IDs.
...
- RoboCasa catalog coverage: `{robocasa_catalog.get('verified_artifact_task_count')}` / `{robocasa_catalog.get('registry_count')}` registered task IDs covered by verified rollout-pool artifacts.
...
- Full RoboCasa-wide learned-WAM rollout collection beyond the current pick-place-family, broad atomic-manipulation, 12-task, and 24-task family artifacts.
