...
- The strongest contact-rich external evidence is Gymnasium Robotics Fetch plus Meta-World ML1, RoboSuite Panda, ManiSkill state mode, RoboCasa three-task pick-place and broad four-task atomic-manipulation artifacts, and LIBERO Spatial rollout-pool validation, but still not real hardware.
...
- Gymnasium/MuJoCo, Gymnasium Robotics Fetch, Meta-World ML1, RoboSuite Panda, RGB WAM-lite on Reacher-v5 and Fetch frames, ManiSkill3 state-mode, RoboCasa three-task pick-place-family plus broad four-task kitchen learned-WAM artifacts, and LIBERO Spatial three-task rollout-pool learned-WAM artifacts validate the benchmark path without claiming hardware evidence.
...
- LIBERO solved-task policy performance and full RoboCasa-wide learned-WAM validation.
...
- LIBERO solved-task policy validation or full RoboCasa-wide validation.
...
Benchmark-full plus Fetch, Meta-World, RoboSuite, ManiSkill state-mode, RoboCasa single-task, three-task pick-place-family, and broad four-task learned WAM-lite, LIBERO Spatial three-task rollout-pool WAM-lite, visual-, blocker-probe-, and audit-validated: learned-toy, multi-env toy validation, Gymnasium/MuJoCo Reacher-v5 benchmark validation, Gymnasium Robotics Fetch validation, Meta-World ML1 manipulation validation, RoboSuite Panda manipulation validation, ManiSkill3 state-mode manipulation validation, RoboCasa kitchen smoke plus single-task, three-task, and broad task family learned-WAM validation, LIBERO rollout-pool learned-WAM validation, toy visual mode, Reacher RGB WAM-lite, Fetch RGB WAM-lite, ManiSkill visual/EE-control blocker probing, and inference-value audit framework artifacts.
...
The project still lacks real robot artifacts, LIBERO solved-task policy validation, full RoboCasa-wide learned-WAM validation, and ManiSkill visual or EE-control validation.

## 7. Current Answer

The repo answers the mathematical and controlled toy-science objections with tests, multi-env artifacts, learned WAM-lite backbones, falsification, an anti-overclaim system, a state-based Gymnasium/MuJoCo benchmark, a three-task Gymnasium Robotics Fetch benchmark, a three-task Meta-World ML1 benchmark, a three-task RoboSuite Panda benchmark, a three-task ManiSkill3 state-mode benchmark, RoboCasa kitchen pick-place and broad atomic-manipulation learned-WAM artifacts, and a three-task LIBERO Spatial rollout-pool learned-WAM artifact. It does not yet answer real-robot realism.

## 8. Unresolved

- LIBERO solved-task policy-performance evaluation beyond dense rollout-pool utility.
- Full RoboCasa-wide learned-WAM rollout collection beyond the current pick-place-family and broad atomic-manipulation artifacts.
...
Substantially stronger after Gymnasium Robotics Fetch, Meta-World ML1, RoboSuite Panda, ManiSkill state-mode validation, pick-place and broad RoboCasa learned-WAM validation, and three-task LIBERO rollout-pool validation. Still not a real-robot paper and still weaker than a benchmark-heavy robotics paper with solved-task LIBERO policy performance, full RoboCasa-wide validation, or RGB-D manipulation.

## 11. Single Highest-Value Next Step

Add solved-task LIBERO policy evaluation or full RoboCasa-wide task coverage next; ManiSkill RGB/RGB-D WAM validation remains the other high-value path if the local SAPIEN/Vulkan blocker is cleared.
