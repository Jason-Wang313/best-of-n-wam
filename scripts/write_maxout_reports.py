...
def main() -> None:
    ...
    robocasa_atomic_gap_probe = load_json("benchmark_robocasa_residual_frontier_sweep_atomic_gap_probe.json")
    ...
    maniskill_dependency_command = (
        "python experiments/benchmark_maniskill_dependency_probe.py --attempt-source-install"
        if maniskill_dependency_probe.get("source_install_attempted")
        else "python experiments/benchmark_maniskill_dependency_probe.py --quick-timeout-s 60"
    )
...
- RoboCasa atomic gap probe: attempted `{robocasa_atomic_gap_probe.get('candidate_task_count')}` uncovered task IDs; timed-out chunks `{robocasa_atomic_gap_probe.get('timed_out_chunk_count')}`; nondegenerate task IDs `{robocasa_atomic_gap_probe.get('nondegenerate_task_count')}`; promoted `{robocasa_atomic_gap_probe.get('verified')}`.
...
- ManiSkill dependency probe: Pinocchio import `{maniskill_dependency_probe.get('pinocchio_import_available')}`; robotics API `{maniskill_dependency_probe.get('pinocchio_api_available')}`; external env Python count `{maniskill_dependency_probe.get('external_env_python_count')}`; external env robotics API any `{maniskill_dependency_probe.get('external_env_pinocchio_api_any_available')}`; binary PyPI `pinocchio` wheel `{maniskill_dependency_probe.get('pypi_pinocchio_binary_wheel_available')}`; binary `pin` wheel `{maniskill_dependency_probe.get('pin_binary_wheel_available')}`; source install attempted `{maniskill_dependency_probe.get('source_install_attempted')}`.
...
- ManiSkill EE-control remains unavailable locally because the robotics Pinocchio API is absent and the `pin` dependency path lacks binary wheels for this Windows/Python stack; the probe distinguishes this from the unrelated small PyPI package named `pinocchio`.
...
- `ROBOCASA_PYTHON experiments/benchmark_robocasa_residual_frontier_sweep.py --summary-tag atomic_gap_probe ...`: attempted `{robocasa_atomic_gap_probe.get('candidate_task_count')}` additional uncovered atomic/control task IDs; completed chunks `{robocasa_atomic_gap_probe.get('completed_chunk_count')}`; timed-out chunks `{robocasa_atomic_gap_probe.get('timed_out_chunk_count')}`; nondegenerate task IDs `{robocasa_atomic_gap_probe.get('nondegenerate_task_count')}`; not promoted when verified is `{robocasa_atomic_gap_probe.get('verified')}`.
...
- `{maniskill_dependency_command}`: passed as a blocker probe; Pinocchio import `{maniskill_dependency_probe.get('pinocchio_import_available')}`, robotics API `{maniskill_dependency_probe.get('pinocchio_api_available')}`, external env Python count `{maniskill_dependency_probe.get('external_env_python_count')}`, external env robotics API any `{maniskill_dependency_probe.get('external_env_pinocchio_api_any_available')}`, binary PyPI `pinocchio` wheel `{maniskill_dependency_probe.get('pypi_pinocchio_binary_wheel_available')}`, binary `pin` wheel `{maniskill_dependency_probe.get('pin_binary_wheel_available')}`, source install attempted `{maniskill_dependency_probe.get('source_install_attempted')}`.
...
- `python scripts/ideal_claim_boundary.py --fail-on-error`: passed with `{ideal_claim_boundary.get('n_ideal_claims')}` ideal claims, `{ideal_claim_boundary.get('n_promotable_claims')}` promotable claims, `{ideal_claim_boundary.get('n_future_only_claims')}` future-only claims, `{ideal_claim_boundary.get('all_ideal_claims_promotable')}` all-promotable flag, and `{ideal_claim_boundary.get('n_issues')}` issues.
