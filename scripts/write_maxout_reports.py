...
- Gymnasium/MuJoCo Reacher-v5, Gymnasium Robotics Fetch, Meta-World ML1, RoboSuite Panda, ManiSkill3 state-mode tasks, RoboCasa kitchen smoke plus single-task, three-task pick-place-family, broad four-task, and 12-task family learned-WAM artifacts, LIBERO Spatial rollout-pool WAM artifacts, a LIBERO Object sparse-success scripted smoke, a LIBERO learned action-head smoke, and a LIBERO time-conditioned autonomous low-dimensional BC sparse-success smoke now have external benchmark artifacts.
- No real robot, DreamZero, UWM, or broad image/language LIBERO policy result is claimed; RoboCasa is verified for pick-place-family, broad atomic kitchen, and 12-task family rollout-pool artifacts, and LIBERO is verified as a three-task rollout-pool dense-utility artifact plus narrow scripted, learned action-head, and time-conditioned low-dimensional BC sparse-success smokes.
...
- Broad image/language LIBERO policy artifacts are still missing; current LIBERO evidence is a three-task Spatial rollout-pool WAM-lite artifact with dense progress utility, a hand scripted Object sparse-success smoke, a learned action-head smoke with scripted phases and target points, and a time-conditioned low-dimensional BC smoke without phase labels or target commands at evaluation time.
...
- README must state LIBERO as optional/separate-environment and limited to three Spatial rollout-pool dense-utility validation plus Object sparse-success scripted, learned action-head, and time-conditioned low-dimensional BC smokes; RoboCasa is optional/separate-environment and includes pick-place-family, broad atomic-manipulation, and 12-task family rollout-pool validation, not full RoboCasa-wide validation.
...
- LIBERO learned action-head smoke: ridge action head trained on `{libero_action_head.get('train_examples')}` scripted action examples and evaluated on `{libero_action_head.get('eval_episodes')}` heldout sparse-success episodes over the scripted-success Object subset.
- LIBERO time-conditioned autonomous low-dimensional BC smoke: kNN behavior cloning trained on `{libero_autonomous_bc.get('train_examples')}` scripted action examples and evaluated on `{libero_autonomous_bc.get('eval_episodes')}` heldout sparse-success episodes over the scripted-success Object subset without phase labels or target-point commands at evaluation time.
...
The single highest reviewer-risk gap is absence of real-robot evidence and broad image/language autonomous benchmark policy validation. LIBERO is now present as a three-task dense-utility rollout-pool WAM artifact plus sparse-success scripted, learned action-head, and time-conditioned low-dimensional BC Object smokes, but not full LIBERO policy evidence.
"""
...
- Broad image/language sparse-success LIBERO policy validation or full RoboCasa-wide validation.
