...
NARRATIVE_SURFACES = [
    ...
    ("abstract_claim_support_report", REPORTS / "abstract_claim_support_report.md"),
    ("publication_scope_report", REPORTS / "publication_scope_report.md"),
...
def main() -> None:
    ...
    external_env_pinocchio_api_available = maniskill_dependency_probe.get("external_env_pinocchio_api_any_available", True)
    add(
        claims,
        68,
        ...
        status(
            ...
            and not external_env_pinocchio_api_available
        ...
        f"external_env_pinocchio_api_any={maniskill_dependency_probe.get('external_env_pinocchio_api_any_available')}, "
    ...
    evidence_hash_coverage_claim = {
        "id": 120,
        "claim": "Non-self claim evidence artifacts and non-self published artifact references have deterministic hash coverage.",
        "status": status(
            bool(evidence_hash_coverage)
            and evidence_hash_coverage.get("verified", False)
            and (evidence_hash_coverage.get("n_claim_sources") or 0) >= 100
            and (evidence_hash_coverage.get("n_artifact_references") or 0) >= 400
            and (evidence_hash_coverage.get("n_hashed_records") or 0) >= 500
            and evidence_hash_coverage.get("n_issues") == 0
            and artifact_exists(RESULTS / "evidence_hash_coverage.json")
            and artifact_exists(REPORTS / "evidence_hash_coverage_report.md"),
            bool(evidence_hash_coverage),
        ),
        "evidence": (
            f"claim_sources={evidence_hash_coverage.get('n_claim_sources')}, "
            f"artifact_refs={evidence_hash_coverage.get('n_artifact_references')}, "
    ...
    abstract_claim_support_claim = {
        "id": 122,
        "claim": "Final-report abstract-level claims are directly evidence-backed and scope-limited.",
        "status": status(
            bool(abstract_claim_support)
            and abstract_claim_support.get("verified", False)
            and abstract_claim_support.get("n_abstract_claims") == 4
            and abstract_claim_support.get("n_approved_abstract_claims") == 4
            and (abstract_claim_support.get("n_backing_claim_links") or 0) >= 20
            and abstract_claim_support.get("n_forbidden_headline_hits") == 0
            and abstract_claim_support.get("n_issues") == 0
            and artifact_exists(RESULTS / "abstract_claim_support.json")
            and artifact_exists(REPORTS / "abstract_claim_support_report.md"),
            bool(abstract_claim_support),
        ),
        "evidence": (
            f"abstract_claims={abstract_claim_support.get('n_abstract_claims')}, "
            f"approved={abstract_claim_support.get('n_approved_abstract_claims')}, "
            f"backing_links={abstract_claim_support.get('n_backing_claim_links')}, "
            f"headline_forbidden={abstract_claim_support.get('n_forbidden_headline_hits')}, "
            f"issues={abstract_claim_support.get('n_issues')}"
        ),
    }
    publication_scope_claim = {
        "id": 123,
        "claim": "Publication-surface risky robotics claims are guarded as limitations or future work.",
        "status": status(
            bool(publication_scope)
            and publication_scope.get("verified", False)
            and (publication_scope.get("n_publication_surfaces") or 0) >= 5
            and (publication_scope.get("n_risk_mentions") or 0) >= 20
            and publication_scope.get("n_unguarded_mentions") == 0
            and publication_scope.get("n_issues") == 0
            and artifact_exists(RESULTS / "publication_scope.json")
            and artifact_exists(REPORTS / "publication_scope_report.md"),
            bool(publication_scope),
        ),
        "evidence": (
            f"surfaces={publication_scope.get('n_publication_surfaces')}, "
            f"risk_mentions={publication_scope.get('n_risk_mentions')}, "
            f"guarded={publication_scope.get('n_guarded_mentions')}, "
            f"unguarded={publication_scope.get('n_unguarded_mentions')}, "
    ...
    report_generation_consistency_claim = {
        "id": 118,
        "claim": "Generated max-out narrative reports are byte-stable under rerunning the report generator.",
        "status": status(
            bool(report_generation_consistency)
            and report_generation_consistency.get("verified", False)
            and (report_generation_consistency.get("n_reports") or 0) >= 8
            and (report_generation_consistency.get("n_files_checked") or 0) >= 8
            and (report_generation_consistency.get("n_checks") or 0) >= 9
            and report_generation_consistency.get("n_issues") == 0
            and artifact_exists(RESULTS / "report_generation_consistency.json")
            and artifact_exists(REPORTS / "report_generation_consistency_report.md"),
            bool(report_generation_consistency),
        ),
        "evidence": (
            f"reports={report_generation_consistency.get('n_reports')}, files={report_generation_consistency.get('n_files_checked')}, "
            f"checks={report_generation_consistency.get('n_checks')}, issues={report_generation_consistency.get('n_issues')}, "
