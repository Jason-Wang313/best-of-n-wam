...
RoboCasa is integrated as an optional single-task artifact on `robocasa/PickPlaceCounterToCabinet`. The local run used a separate RoboCasa-compatible virtual environment and the official kitchen assets. The smoke layer uses five clone-restored rollout pools with 80 random action-sequence rollouts, exact-law utility MAE `0.00027`, and oracle-minus-random N8 CI lower `0.1515`. A lightweight ridge state/action-sequence WAM-lite is trained on 80 RoboCasa rollouts, reports validation utility correlation `0.764`, and beats random on five heldout rollout pools with learned-minus-random N8 CI lower `0.0503`. This supports a contact-rich single-task learned-WAM sanity check, not a multi-task RoboCasa benchmark.
...
Benchmark RGB WAM-lite validation is integrated on Gymnasium/MuJoCo `Reacher-v5` and Gymnasium Robotics Fetch, where RGB rendering works. The model consumes rendered RGB frame features and action sequences, predicts rollout utility, and is evaluated with exact-law and best-of-N scorer-comparison artifacts. ManiSkill RGB/RGB-D WAM validation is not claimed because the local SAPIEN/Vulkan renderer failed during RGB observation creation; the exact probe matrix and errors are saved in `results/benchmark_maniskill_visual_probe.json` and `reports/maniskill_visual_blocker_report.md`. LIBERO remains engineering scaffolding until dependencies and task adapters are completed; RoboCasa remains single-task until multi-task artifacts exist.
