...
RoboCasa is integrated only as an optional smoke artifact on `robocasa/PickPlaceCounterToCabinet`. The local run used a separate RoboCasa-compatible virtual environment, the official kitchen assets, one clone-restored rollout pool with 16 random action-sequence rollouts, exact-law utility MAE `0.00018`, and a tiny closed-loop trace. This supports a contact-rich reset/rollout sanity check, not a full RoboCasa learned-WAM benchmark.
