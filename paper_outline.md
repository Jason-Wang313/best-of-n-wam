...
RoboCasa is integrated only as an optional smoke artifact on `robocasa/PickPlaceCounterToCabinet`. The local run used a separate RoboCasa-compatible virtual environment, the official kitchen assets, five clone-restored rollout pools with 80 random action-sequence rollouts, exact-law utility MAE `0.00027`, and oracle-minus-random N8 CI lower `0.1515`. A tiny closed-loop trace is saved, but not promoted as validation. This supports a contact-rich reset/rollout sanity check, not a full RoboCasa learned-WAM benchmark.
