...
LIBERO is integrated as an optional separate-environment artifact in three layers. The WAM-lite layer covers three `libero_spatial` tasks: a ridge state/action-sequence model trains on 192 rollout samples, validates on 96 samples with utility correlation `0.353`, evaluates on 240 heldout rollout samples from 15 rollout pools, has exact-law utility MAE `0.00014`, and the promoted learned energy-regularized scorer beats random at N8 with CI lower `0.265`. A separate sparse-success smoke runs a hand scripted OSC pick-place controller on all 10 `libero_object` tasks over 5 seeds and obtains 30 successes over 50 episodes, with success-rate CI `[0.46, 0.74]`. A third artifact trains a ridge action head on 2,580 scripted action examples and obtains 18/18 heldout sparse successes on the six-task scripted-success Object subset, with success-rate CI `[1.0, 1.0]`. This supports LIBERO rollout-pool dense-utility validation, a narrow real sparse-success simulator smoke, and a learned action-head smoke with scripted phases and target points; it is not autonomous learned policy performance or full LIBERO validation. VERIFIED CLAIM 86. VERIFIED CLAIM 87.
