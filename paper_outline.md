...
RoboCasa is integrated as an optional contact-rich kitchen artifact in a separate RoboCasa-compatible virtual environment with official kitchen assets. The single-task layer uses `robocasa/PickPlaceCounterToCabinet`, five clone-restored rollout pools with 80 random action-sequence rollouts, exact-law utility MAE `0.00027`, oracle-minus-random N8 CI lower `0.1515`, and a lightweight ridge state/action-sequence WAM-lite with validation utility correlation `0.764` and learned-minus-random N8 CI lower `0.0503`. The pick-place-family layer is a three-task task conditioned WAM-lite over `PickPlaceCounterToCabinet`, `PickPlaceCounterToDrawer`, and `PickPlaceCounterToMicrowave`: 144 train rollouts, 96 validation rollouts, validation utility correlation `0.675`, 240 heldout eval rollouts, exact-law MAE `0.00035`, and learned energy-regularized scorer minus random N8 CI lower `0.169`. The broad atomic-manipulation layer covers `OpenDrawer`, `OpenCabinet`, `OpenMicrowave`, and `TurnOnSinkFaucet`: 64 train rollouts, 32 validation rollouts, validation utility correlation `0.860`, 128 heldout eval rollouts across 16 rollout pools, exact-law MAE `0.00036`, and learned scorer minus random N8 CI lower `0.211`. The 12-task family layer covers open/close drawer, cabinet, microwave, oven plus sink/stove on/off tasks: 96 train rollouts, 96 validation rollouts, validation utility correlation `0.833`, 192 heldout eval rollouts across 24 rollout pools, exact-law MAE `0.00029`, and learned energy-regularized scorer minus random N8 CI lower `0.183`. The 24-task family layer covers open/close/turn and pick-place variants: 192 train rollouts, 192 validation rollouts, validation utility correlation `0.852`, 384 heldout eval rollouts across 48 rollout pools, exact-law MAE `0.00027`, and learned energy-regularized scorer minus random N8 CI lower `0.239`. The extra four-task layer covers `PickPlaceCounterToStandMixer`, `PickPlaceCounterToToasterOven`, `PickPlaceDrawerToCounter`, and `PickPlaceMicrowaveToCounter`: 64 train rollouts, 32 validation rollouts, validation utility correlation `0.799`, 128 heldout eval rollouts across 16 rollout pools, exact-law MAE `0.00030`, and learned scorer minus random N8 CI lower `0.247`. The combined 28-task layer trains one task aware WAM over the 24-task family plus the four extra pick-place directions: 448 train rollouts, 224 validation rollouts, validation utility correlation `0.834`, 448 heldout eval rollouts across 56 rollout pools, exact-law MAE `0.00027`, and learned scorer minus random N8 CI lower `0.235`. The combined 32-task layer adds `CloseFridgeDrawer`, `TurnOnBlender`, `PickPlaceCabinetToCounter`, and `PickPlaceSinkToCounter`: 512 train rollouts, 256 validation rollouts, validation utility correlation `0.838`, 512 heldout eval rollouts across 64 rollout pools, exact-law MAE `0.00027`, and learned energy-regularized scorer minus random N8 CI lower `0.228`. The stratified 55-task layer spans opening, closing, turning, pick-place, cleaning, washing/rinsing, cooking/control, transport, filling, loading, arranging, gathering, and organizing task IDs: 880 train rollouts, 440 validation rollouts, validation utility correlation `0.833`, 880 heldout eval rollouts across 110 rollout pools, exact-law MAE `0.00028`, and learned scorer minus random N8 CI lower `0.274`. The stratified 97-task frontier layer extends the 55-task set with 42 manipulation, movement, pick-place, cleanup, washing, cooking, arrangement, and compositional kitchen IDs: 1,552 train rollouts, 776 validation rollouts, validation utility correlation `0.838`, 1,552 heldout eval rollouts across 194 rollout pools, exact-law MAE `0.00027`, learned scorer minus random N8 CI lower `0.315`, and oracle-minus-learned N8 CI lower `0.0369`. Separate micro-rollout preflights verify reset/clone/short-rollout viability for the extra four pick-place task IDs, 23 stratified kitchen task IDs, and 42 frontier task IDs from 64 attempted candidates. A registry audit finds 396 local RoboCasa task IDs, 97 task IDs covered by verified rollout-pool artifacts, 69 task IDs covered by micro-rollout probes, and 97 task IDs covered by any committed artifact. These support RoboCasa rollout-pool dense-utility results across pick-place and non-pick-place task families, not full RoboCasa-wide validation or solved policies. VERIFIED CLAIM 91. VERIFIED CLAIM 92. VERIFIED CLAIM 93. VERIFIED CLAIM 94. VERIFIED CLAIM 95. VERIFIED CLAIM 96. VERIFIED CLAIM 97.
