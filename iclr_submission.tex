...
\section{V3 Evidence Audit}
\label{sec:v3-audit}

The v3 paper adds a cached evidence layer over the repository artifacts. The
script \path{experiments/v3_cached_evidence.py} reads the committed theorem,
mismatch, allocation, robotics, RoboCasa, LIBERO, claim-ledger, and artifact
manifest summaries. It writes compact CSV ledgers, five figures, a summary JSON
...
The current v3 ledger reports \VThreeWAMClaimsVerified{} verified claim checks,
\VThreeWAMClaimsPartial{} partial claim checks, and
\VThreeWAMClaimsUnsupported{} unsupported claim checks under the repository's
claim-status script. The artifact manifest covers
\VThreeWAMArtifactFiles{} files, including \VThreeWAMArtifactCSV{} CSV tables
and \VThreeWAMArtifactJSON{} JSON summaries. These counts are not a claim of
...
\begin{figure}[t]
\centering
\includegraphics[width=0.78\linewidth]{paper_figures/v3/v3_exact_law_errors.pdf}
...
artifacts.}
\label{fig:v3-exact-errors}
\end{figure}

Figure~\ref{fig:v3-exact-errors} summarizes the theorem-audit role of the
benchmark artifacts. In the core analytic check, the mean success Monte Carlo
MAE is \VThreeWAMExactSuccessMAE{} and the mean utility MAE is
\VThreeWAMExactUtilityMAE{}. In rollout-pool benchmarks, exact-law utility
MAEs include \VThreeWAMFetchExactMAE{} for Fetch state WAM,
\VThreeWAMFetchRGBExactMAE{} for Fetch RGB WAM,
\VThreeWAMMetaWorldExactMAE{} for Meta-World,
\VThreeWAMRoboSuiteExactMAE{} for RoboSuite, and
\VThreeWAMManiSkillExactMAE{} for ManiSkill state-mode.

\begin{figure}[t]
\centering
\includegraphics[width=0.80\linewidth]{paper_figures/v3/v3_benchmark_ci_lowers.pdf}
\caption{\textbf{Promoted scorer gains over random.} The v3 ledger reports CI
...
\VThreeWAMFetchCILo{} at $N=32$, while Fetch RGB WAM has visual-minus-random CI
lower \VThreeWAMFetchRGBCILo{}. RoboCasa's stratified
\VThreeWAMRoboCasaTasks{}-task artifact evaluates
\VThreeWAMRoboCasaEvalSamples{} heldout rollouts across
\VThreeWAMRoboCasaPools{} rollout pools, with utility correlation
\VThreeWAMRoboCasaCorr{} and promoted learned-minus-random CI lower
...
rollout pools, with utility correlation \VThreeWAMRoboCasaResidualCorr{} and
CI lower \VThreeWAMRoboCasaResidualCILo{}. LIBERO Spatial contributes
...
\begin{figure}[t]
\centering
\includegraphics[width=0.76\linewidth]{paper_figures/v3/v3_failure_modes.pdf}
...
scoring failures.}
\label{fig:v3-failure}
\end{figure}

The failure-mode ledger is equally important. Severe imagined-real mismatch
has gap growth \VThreeWAMSevereGapGrowth{}, and the stuck/slip mismatch has
gap growth \VThreeWAMStuckGapGrowth{}. The anti-real-utility scorer changes
selected utility from \VThreeWAMAntiScorerNOne{} at $N=1$ to
\VThreeWAMAntiScorerNSixtyFour{} at $N=64$. These are not theorem failures.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.76\linewidth]{paper_figures/v3/v3_robocasa_coverage.pdf}
\caption{\textbf{RoboCasa coverage accounting.} Coverage is broad but not
complete; v3 keeps full-RoboCasa claims out of the headline.}
\label{fig:v3-robocasa}
\end{figure}

RoboCasa coverage is explicitly counted rather than implied. The local registry
contains \VThreeWAMRoboCasaRegistered{} task IDs. Verified learned-WAM
rollout-pool artifacts cover \VThreeWAMRoboCasaRolloutTaskIDs{} task IDs, and
any committed rollout-pool or micro-rollout artifact covers
\VThreeWAMRoboCasaAnyTaskIDs{} task IDs. This is strong breadth for a
rollout-pool audit paper, but it is not full RoboCasa-wide validation.

\begin{figure}[t]
\centering
\includegraphics[width=0.74\linewidth]{paper_figures/v3/v3_claim_artifact_counts.pdf}
