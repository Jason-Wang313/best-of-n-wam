...
\section{V3 Cached Evidence Ledger}
\label{app:v3-ledger}

The v3 evidence layer is a paper-facing cache over the repository's existing
...
\path{experiments/v3_cached_evidence.py} performs that role. It reads committed
JSON summaries and CSV ledgers, writes compact v3 tables, creates five figures,
and writes \path{v3_results_macros.tex}. The manuscript imports those macros
...
slow, fragile, and unnecessary. V3 therefore treats the committed artifacts as
the evidence base and makes their use explicit. A future v4 can add new
artifacts and a new evidence cache, but v3 should not silently mutate its claim
surface.
...
the v3 paper. The columns are deliberately chosen to separate three questions:
...
\midrule
Fetch state WAM & 60 & \VThreeWAMFetchExactMAE{} & \VThreeWAMFetchCILo{} &
Gymnasium Robotics state/action rollout pools. \\
Fetch RGB WAM & 45 & \VThreeWAMFetchRGBExactMAE{} & \VThreeWAMFetchRGBCILo{} &
RGB-frame/action rollout pools. \\
Meta-World & 45 & \VThreeWAMMetaWorldExactMAE{} & positive in reward and
learned-score reports & ML1 reach, push, drawer-open. \\
RoboSuite & 30 & \VThreeWAMRoboSuiteExactMAE{} & positive learned/reward
reports & Panda Lift, Stack, Door. \\
ManiSkill state & 30 & \VThreeWAMManiSkillExactMAE{} & dense-score positive,
learned score mixed & CPU state-mode joint-delta control. \\
RoboCasa 97-task & \VThreeWAMRoboCasaPools{} & $0.0003$ &
\VThreeWAMRoboCasaCILo{} & Stratified kitchen-family rollout pools. \\
RoboCasa residual 35 & \VThreeWAMRoboCasaResidualPools{} & $0.0002$ &
\VThreeWAMRoboCasaResidualCILo{} & Clean/cook residual rollout pools. \\
LIBERO Spatial & \VThreeWAMLiberoPools{} & $0.0001$ & \VThreeWAMLiberoCILo{} &
Three-task rollout-pool WAM-lite. \\
\bottomrule
\end{tabular}
\caption{V3 benchmark evidence matrix. Exact MAE checks the tied-pool law;
...
the assumptions are mismatched. The v3 evidence reports low exact-law errors
...
The v3 claim policy has four tiers. Tier 1 is exact score-tail accounting.
...
\VThreeWAMFetchExactMAE{}, and the learned-minus-random $N=32$ CI lower is
\VThreeWAMFetchCILo{}. The RGB-frame/action variant reports exact-law MAE
\VThreeWAMFetchRGBExactMAE{} and CI lower \VThreeWAMFetchRGBCILo{}. These
...
\VThreeWAMMetaWorldExactMAE{}. The learned score is weaker than reward-like
...
\VThreeWAMRoboSuiteExactMAE{}, and reports include positive learned/reward
...
\VThreeWAMManiSkillExactMAE{}. Visual and end-effector-control validation are
not claimed because the local stack exposes renderer and robotics-API blockers.
This is not a detail to bury. It is an example of v3's claim discipline:
...
\VThreeWAMRoboCasaTasks{} tasks, \VThreeWAMRoboCasaPools{} rollout pools, and
\VThreeWAMRoboCasaEvalSamples{} heldout rollouts, with utility correlation
\VThreeWAMRoboCasaCorr{} and learned-minus-random CI lower
\VThreeWAMRoboCasaCILo{}. The residual clean/cook artifact covers
\VThreeWAMRoboCasaResidualTasks{} tasks and \VThreeWAMRoboCasaResidualPools{}
rollout pools, with CI lower \VThreeWAMRoboCasaResidualCILo{}. Coverage
accounting reports \VThreeWAMRoboCasaRolloutTaskIDs{} rollout-pool task IDs and
\VThreeWAMRoboCasaAnyTaskIDs{} any-artifact task IDs out of
\VThreeWAMRoboCasaRegistered{} registered IDs. This is broad, not complete.

\paragraph{LIBERO.}
The LIBERO Spatial rollout-pool WAM-lite artifact covers
\VThreeWAMLiberoPools{} rollout pools, with utility correlation
\VThreeWAMLiberoCorr{} and learned-energy-regularized CI lower
\VThreeWAMLiberoCILo{}. Separate LIBERO Object sparse-success smokes show that
scripted or lightweight behavior-cloned policies can solve those local sparse
success checks, but v3 does not present them as modern VLA policy performance.
...
utility degrades, producing gap growth \VThreeWAMSevereGapGrowth{}. In the
stuck/slip artifact, the corresponding gap growth is
\VThreeWAMStuckGapGrowth{}. These results target the precise WAM risk:
...
\VThreeWAMAntiScorerNOne{} at $N=1$ to
\VThreeWAMAntiScorerNSixtyFour{} at $N=64$. This is exactly what the law should
...
\VThreeWAMAdaptiveGain{} at the reference budget.
...
The v3 paper uses exact identities where possible and confidence intervals
...
scope note for unavailable modalities. V3 already follows this spirit through
its artifact reports and cached evidence layer.

\section{RAM-Light Execution Strategy}
\label{app:ram-light}

The v3 build is deliberately RAM-light. It does not load image datasets, replay
...
\item The title used to sound like a generic Best-of-$N$ wrapper. V3 avoids
...
\item Severe mismatch could be dismissed as toy-only. V3 pairs it with learned
...
\item The artifact base is large but could be hard to inspect. V3 adds compact
evidence ledgers.
\item The manuscript was only 11 pages before v3. V3 adds a full evidence and
claim-boundary appendix.
\item The final artifact must be versioned. The v3 build writes a versioned
PDF.
...
\item Source-map consistency matters. The v3 audit checks it.
...
PDF pages.
\item The claim-status count could become stale. The v3 evidence script reads
the generated claim-status JSON.
\item The artifact manifest could become stale. The v3 audit checks summary
counts.
...
attack.
\item The paper may be too broad. V3 uses claim tiers to contain the breadth.
\item The paper may be too artifact-heavy. V3 ledgers make the artifacts
auditable.
...
stale profiles.
\item The figures need real artifact links. The v3 ledger names source JSON
and CSV files.
\item The Desktop artifact must be easy for a fresh agent to find. The source
map and archive contract address this.
\item The final PDF must clear 25 pages. The v3 audit checks the page count.
\item Old v2 Desktop copies must not be the final paper. The v3 workflow
copies the v3 PDF and updates the source map.
...
\begin{enumerate}
\item The v3 evidence script regenerates \path{results/v3_cached_evidence/},
\path{paper_figures/v3/}, and \path{v3_results_macros.tex}.
\item The paper compiles from the current source with bibliography and stable
cross-references.
\item The final repository PDF is
\path{paper/final/best-of-n-wam-v3.pdf}.
\item The visible Desktop PDF is
\path{best-of-n-wam-v3.pdf}.
\item Repository and Desktop PDFs have identical SHA-256 hashes.
\item The final PDF has at least 25 pages.
\item The source map row points to the v3 Desktop PDF, this local folder, and
the GitHub repository.
\item The v3 audit script checks summary counts, exact-law values, key
...
The final v3 paper artifact is \path{best-of-n-wam-v3.pdf}. The final source
...
The archive also preserves claim boundaries. V3 should contain the generated
...
artifact and a new evidence cache rather than stretching v3 claims after the
fact.
...
stated, and whether narrative surfaces contain unsupported phrases. The v3
evidence layer reads the generated claim-status JSON and reports
\VThreeWAMClaimsVerified{} verified claims with no partial or unsupported claim
...
manuscript to cite a result too broadly or to forget a blocker. The v3 claim
...
The artifact manifest reports \VThreeWAMArtifactFiles{} files totaling
\VThreeWAMArtifactTotalMB{} MB, with \VThreeWAMArtifactCSV{} CSV tables and
\VThreeWAMArtifactJSON{} JSON summaries. The count itself is not a result. Its
role is to make the paper's provenance auditable. A reviewer or future agent
can inspect the manifest to see which files were present when v3 was built and
...
generated summaries. The v3 script reads JSON files such as
...
claims. Reports from earlier phases remain useful historical records, but v3
should point to the generated v3 evidence cache and current final PDF. If a
...
blocked on the local renderer and robotics dependency stack. The v3 paper does
...
lightweight behavior-cloning smoke. V3 therefore separates LIBERO Spatial
...
end-effector-control paths are not promoted. V3 treats this as a strength of
...
The reproduction protocol has two layers. The first layer is the ordinary v3
paper-build layer. It requires Python, the repository dependencies,
\LaTeX{}/BibTeX, and the committed results. Running
\path{python scripts/build_v3_paper.py} regenerates the v3 evidence cache,
figures, macros, unversioned \LaTeX{} output, repository final PDF, and Desktop
copy. Running the v3 audit then checks page count, hashes, source map,
...
install the compatible simulator stack. V3 does not require this layer for
...
kind of evidence behind it. V3 therefore separates claim support into
...
v3 evidence cache reports binary success MAE
\VThreeWAMExactSuccessMAE{} and utility MAE \VThreeWAMExactUtilityMAE{} in the
...
and a high-$N$ same-AUC gap of \VThreeWAMSameAUCGap{}. The evidence is not
...
\VThreeWAMSevereGapGrowth{}, stuck/slip mismatch produces gap growth
\VThreeWAMStuckGapGrowth{}, and the anti-scorer moves from
\VThreeWAMAntiScorerNOne{} at $N=1$ to \VThreeWAMAntiScorerNSixtyFour{} at
...
reference allocation gain is \VThreeWAMAdaptiveGain{}. The paper does not claim
...
The v3 cache aggregates \VThreeWAMBenchmarkRows{} benchmark rows, including
state, visual, manipulation, kitchen-family, and LIBERO rollout-pool artifacts.
RoboCasa coverage is particularly broad, with \VThreeWAMRoboCasaTasks{} tasks,
\VThreeWAMRoboCasaPools{} rollout pools, and
\VThreeWAMRoboCasaEvalSamples{} heldout rollouts in the stratified artifact.
The residual clean/cook artifact adds \VThreeWAMRoboCasaResidualTasks{} more
...
This claim is supported by the source map, final PDF naming, v3 build script,
v3 audit script, generated macros, and claim-status ledger. It is a practical
...
strong but operationally confusing. V3 treats operational clarity as part of
...
Fetch is acceptable in v3 if both state and visual variants show low exact-law
...
\VThreeWAMFetchExactMAE{} and \VThreeWAMFetchRGBExactMAE{}, and CI lower
bounds \VThreeWAMFetchCILo{} and \VThreeWAMFetchRGBCILo{}. The acceptance
...
current artifacts report exact-law MAEs \VThreeWAMMetaWorldExactMAE{} and
\VThreeWAMRoboSuiteExactMAE{}. If a learned score is weaker than a reward-like
...
state-mode exact-law MAE \VThreeWAMManiSkillExactMAE{} and discuss dense-score
...
The paper can claim stratified coverage over \VThreeWAMRoboCasaTasks{} tasks
and \VThreeWAMRoboCasaPools{} rollout pools, residual clean/cook coverage over
\VThreeWAMRoboCasaResidualTasks{} tasks, and catalog accounting over
\VThreeWAMRoboCasaRegistered{} registered task IDs. It must also state that
...
branch. The paper can report \VThreeWAMLiberoPools{} pools, correlation
\VThreeWAMLiberoCorr{}, and CI lower \VThreeWAMLiberoCILo{}. It may also
...
score tail is bad. V3 therefore treats negative controls as first-class
...
score groups. The observed movement from \VThreeWAMAntiScorerNOne{} to
\VThreeWAMAntiScorerNSixtyFour{} is therefore not a surprising pathology. It
...
world-model validity. V3 does not blur those layers. It uses exact-law errors
...
states. V3 therefore frames adaptive allocation as an audit-informed heuristic.
...
or equates exact selection accounting with deployment safety. V3 says the
...
The v3 title, abstract, experiments, and appendix are designed to avoid that
failure.
...
v3, necessary experiments are exact-law validation, AUC-boundary validation,
...
If reviewers or future authors want to push the paper beyond v3, the next
...
$N$ and reduce the negative-tail cases documented in v3.

This protocol is included because it prevents v3 from being both too small and
...
Naming correct rejection paths helps v3 avoid dishonest defenses. It also
...
contains many optional branches, but v3 does not inflate them. It reports
...
A second future direction is score repair. V3 audits score tails and includes
...
the v3 LIBERO smoke artifacts. It should create a new evidence tier with its
...
families are covered and why. V3's coverage accounting is a starting point, not
the finish line.
...
\path{best-of-n-wam-v3.pdf}, this local folder, and the GitHub repository.
Second, run \path{git status --short --branch} and verify that the working tree
is clean. Third, inspect \path{paper/final/best-of-n-wam-v3.pdf} rather than
the unversioned build product. Fourth, run \path{python scripts/run_v3_claim_audit.py}
...
artifacts. Without a final source map and v3 audit, a new session could easily
open an old PDF, treat a smoke artifact as final evidence, or confuse optional
runtime probes with promoted claims. The v3 archive contract is meant to
