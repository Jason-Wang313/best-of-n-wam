...
It writes `results/benchmark_maniskill_dependency_probe.json` and `reports/maniskill_dependency_blocker_report.md`. The committed artifact documents that the robotics Pinocchio API is unavailable on this Windows/Python stack and that pip did not expose binary `pin`/`cmeel-boost` wheels, so ManiSkill end-effector-control validation is not claimed. The probe distinguishes that API from the unrelated small PyPI package named `pinocchio`, which is not sufficient for ManiSkill/SAPIEN end-effector-control evidence.
...
Claims are classified as `VERIFIED`, `PARTIAL`, `UNSUPPORTED`, or `FAILED` from artifacts. The claim gate is surrounded by twenty-eight stricter consistency checks:
...
python scripts/claims_status.py
python scripts/abstract_claim_support.py --fail-on-error
python scripts/publication_scope.py --fail-on-error
python scripts/frontier_integrity.py --fail-on-error
python scripts/ideal_claim_boundary.py --fail-on-error
python scripts/claims_status.py
...
python scripts/repo_bound_artifact_audit.py --fail-on-error
python scripts/claims_status.py
