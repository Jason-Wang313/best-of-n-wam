...
- `python scripts/artifact_integrity.py --fail-on-error`: passed with `733` artifact references checked and `0` issues.
- `python scripts/artifact_manifest.py --fail-on-error`: passed with `454` scientific artifacts, `77238314` bytes, `15` manifest checks, and `0` issues.
...
- `python scripts/tracked_artifact_provenance.py --fail-on-error`: passed with `119` claim sources, `475` artifact references, and `0` issues.
