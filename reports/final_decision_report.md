...
- `python -m pytest -q`: passed with `123 passed`.
- `python scripts/test_inventory.py --fail-on-error`: passed with `123` collected tests, `6` inventory checks, and `0` issues.
...
- `python scripts/artifact_integrity.py --fail-on-error`: passed with `685` artifact references checked and `0` issues.
...
- `python scripts/source_manifest.py --fail-on-error`: passed with `203` source files, `1427444` bytes, `14` source-manifest checks, and `0` issues.
...
- `python scripts/script_contracts.py --fail-on-error`: passed with `7` scripts, `125` contract checks, and `0` issues.
...
- `python scripts/claim_semantics.py --fail-on-error`: passed with `125` claims, `175` semantic checks, `53` CI-backed claims, and `0` issues.
- `python scripts/claim_scope_audit.py --fail-on-error`: passed with `125` claims, `155` scoped broad-claim mentions, `160` checks, and `0` issues.
...
- `python scripts/claim_evidence_quality.py --fail-on-error`: passed with `125` claims, `171` source links, `7` evidence checks, and `0` issues.
- `python scripts/tracked_artifact_provenance.py --fail-on-error`: passed with `115` claim sources, `443` artifact references, and `0` issues.
- `python scripts/repo_bound_artifact_audit.py --fail-on-error`: passed with `856` records, `171` claim sources, `685` artifact references, `0` outside-repo records, `0` missing records, and `0` issues.
- `python scripts/claim_ledger_integrity.py --fail-on-error`: passed with `125` claims, `31` ledger checks, and `0` issues.
- `python scripts/claim_generation_consistency.py --fail-on-error`: passed with `125` claims, `10` generation checks, and `0` issues.
- `python scripts/report_generation_consistency.py --fail-on-error`: passed with `8` reports, `9` generation checks, and `0` issues.
- `python scripts/evidence_hash_coverage.py --fail-on-error`: passed with `111` claim sources, `441` artifact references, `552` hashed records, and `0` issues.
- `python scripts/claims_status.py`: passed with `125` verified, `0` partial, `0` unsupported, `0` failed, and `0` README/paper overclaims.
