...
- `python scripts/artifact_manifest.py --fail-on-error`: passed with `423` scientific artifacts, `69778661` bytes, `15` manifest checks, and `0` issues.
...
- `python scripts/table_schema.py --fail-on-error`: passed with `222` tables, `212888` rows, `17` schema checks, and `0` issues.
- `python scripts/source_manifest.py --fail-on-error`: passed with `224` source files, `1628335` bytes, `14` source-manifest checks, and `0` issues.
