...
- `python scripts/artifact_integrity.py --fail-on-error`: passed with `621` artifact references checked and `0` issues.
...
- `python scripts/narrative_consistency.py --fail-on-error`: passed with `35` narrative checks and `0` issues.
- `python scripts/script_contracts.py --fail-on-error`: passed with `7` scripts, `57` contract checks, and `0` issues.
- `python scripts/claim_semantics.py --fail-on-error`: passed with `109` claims, `168` semantic checks, `51` CI-backed claims, and `0` issues.
- `python scripts/claim_evidence_quality.py --fail-on-error`: passed with `109` claims, `139` source links, `7` evidence checks, and `0` issues.
- `python scripts/claim_ledger_integrity.py --fail-on-error`: passed with `109` claims, `31` ledger checks, and `0` issues.
- `python scripts/claims_status.py`: passed with `109` verified, `0` partial, `0` unsupported, `0` failed, and `0` README/paper overclaims.
