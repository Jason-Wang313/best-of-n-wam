...
- `python scripts/script_contracts.py --fail-on-error`: passed with `7` scripts, `37` contract checks, and `0` issues.
- `python scripts/claim_ledger_integrity.py --fail-on-error`: passed with `104` claims, `31` ledger checks, and `0` issues.
- `python scripts/claims_status.py`: passed with `104` verified, `0` partial, `0` unsupported, `0` failed, and `0` README/paper overclaims.
