...
- `python scripts/artifact_integrity.py --fail-on-error`: passed with `727` artifact references checked and `0` issues.
- `python scripts/artifact_manifest.py --fail-on-error`: passed with `444` scientific artifacts, `77213794` bytes, `15` manifest checks, and `0` issues.
...
- `python scripts/source_manifest.py --fail-on-error`: passed with `246` source files, `1793108` bytes, `14` source-manifest checks, and `0` issues.
