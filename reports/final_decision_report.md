...
- 103. Claim ledger is structurally consistent. Evidence: claims=127, max_id=127, checks=21, issues=1
- 115. Published final-report command results match current verification artifacts. Evidence: snippets=28, python_commands=31, pytest=158, checks=31, issues=2
...
- `python scripts/test_inventory.py --fail-on-error`: passed with `158` collected tests, `6` inventory checks, and `0` issues.
...
- `python scripts/artifact_integrity.py --fail-on-error`: passed with `716` artifact references checked and `0` issues.
- `python scripts/artifact_manifest.py --fail-on-error`: passed with `420` scientific artifacts, `63126168` bytes, `15` manifest checks, and `0` issues.
...
- `python scripts/table_schema.py --fail-on-error`: passed with `221` tables, `212881` rows, `17` schema checks, and `0` issues.
- `python scripts/source_manifest.py --fail-on-error`: passed with `224` source files, `1623925` bytes, `14` source-manifest checks, and `0` issues.
...
- `python scripts/experiment_registry.py --fail-on-error`: passed with `57` experiment-family entries, `69` wrapper links, `298` table artifacts, `40` figures, `10` registry checks, and `0` issues.
- `python scripts/model_artifact_integrity.py --fail-on-error`: passed with `50` model artifacts, `338` NPZ arrays, `13` joblib predictors, `10` model-artifact checks, and `0` issues.
...
- `python scripts/tracked_artifact_provenance.py --fail-on-error`: passed with `119` claim sources, `460` artifact references, and `0` issues.
- `python scripts/repo_bound_artifact_audit.py --fail-on-error`: passed with `889` records, `175` claim sources, `714` artifact references, `0` outside-repo records, `0` missing records, and `0` issues.
...
- `python scripts/evidence_hash_coverage.py --fail-on-error`: passed with `117` claim sources, `458` artifact references, `575` hashed records, and `0` issues.
- `python scripts/claims_status.py`: passed with `125` verified, `2` partial, `0` unsupported, `0` failed, and `0` README/paper overclaims.
