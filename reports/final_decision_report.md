...
- `python -m pytest -q`: passed with `98 passed`.
- `python scripts/test_inventory.py --fail-on-error`: passed with `98` collected tests, `6` inventory checks, and `0` issues.
...
- `python scripts/artifact_integrity.py --fail-on-error`: passed with `649` artifact references checked and `0` issues.
...
- `python scripts/source_manifest.py --fail-on-error`: passed with `176` source files, `1264915` bytes, `14` source-manifest checks, and `0` issues.
...
- `python scripts/narrative_consistency.py --fail-on-error`: passed with `41` narrative checks and `0` issues.
- `python scripts/script_contracts.py --fail-on-error`: passed with `7` scripts, `85` contract checks, and `0` issues.
- `python scripts/claim_semantics.py --fail-on-error`: passed with `116` claims, `169` semantic checks, `51` CI-backed claims, and `0` issues.
- `python scripts/claim_evidence_quality.py --fail-on-error`: passed with `116` claims, `153` source links, `7` evidence checks, and `0` issues.
- `python scripts/claim_ledger_integrity.py --fail-on-error`: passed with `116` claims, `31` ledger checks, and `0` issues.
- `python scripts/claims_status.py`: passed with `116` verified, `0` partial, `0` unsupported, `0` failed, and `0` README/paper overclaims.
