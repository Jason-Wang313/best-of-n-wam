...
- `python scripts/test_inventory.py --fail-on-error`: passed with `113` collected tests, `6` inventory checks, and `0` issues.
...
- `python scripts/artifact_integrity.py --fail-on-error`: passed with `677` artifact references checked and `0` issues.
...
- `python scripts/source_manifest.py --fail-on-error`: passed with `194` source files, `1374219` bytes, `14` source-manifest checks, and `0` issues.
...
- `python scripts/narrative_consistency.py --fail-on-error`: passed with `47` narrative checks and `0` issues.
- `python scripts/script_contracts.py --fail-on-error`: passed with `7` scripts, `113` contract checks, and `0` issues.
...
- `python scripts/claim_semantics.py --fail-on-error`: passed with `122` claims, `171` semantic checks, `51` CI-backed claims, and `0` issues.
- `python scripts/claim_evidence_quality.py --fail-on-error`: passed with `122` claims, `165` source links, `7` evidence checks, and `0` issues.
- `python scripts/tracked_artifact_provenance.py --fail-on-error`: passed with `109` claim sources, `439` artifact references, and `0` issues.
- `python scripts/repo_bound_artifact_audit.py --fail-on-error`: passed with `842` records, `165` claim sources, `677` artifact references, `0` outside-repo records, `0` missing records, and `0` issues.
- `python scripts/claim_ledger_integrity.py --fail-on-error`: passed with `122` claims, `31` ledger checks, and `0` issues.
- `python scripts/claim_generation_consistency.py --fail-on-error`: passed with `122` claims, `10` generation checks, and `0` issues.
- `python scripts/report_generation_consistency.py --fail-on-error`: passed with `8` reports, `9` generation checks, and `0` issues.
- `python scripts/evidence_hash_coverage.py --fail-on-error`: passed with `107` claim sources, `437` artifact references, `544` hashed records, and `0` issues.
- `python scripts/claims_status.py`: passed with `122` verified, `0` partial, `0` unsupported, `0` failed, and `0` README/paper overclaims.
