...
- `python -m pytest -q`: passed with `137 passed`.
- `python scripts/test_inventory.py --fail-on-error`: passed with `137` collected tests, `6` inventory checks, and `0` issues.
...
- `python scripts/source_manifest.py --fail-on-error`: passed with `209` source files, `1490169` bytes, `14` source-manifest checks, and `0` issues.
