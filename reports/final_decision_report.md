...
- `python -m pytest -q`: passed with `104 passed`.
- `python scripts/test_inventory.py --fail-on-error`: passed with `104` collected tests, `6` inventory checks, and `0` issues.
...
- `python scripts/artifact_integrity.py --fail-on-error`: passed with `665` artifact references checked and `0` issues.
...
- `python scripts/source_manifest.py --fail-on-error`: passed with `185` source files, `1314286` bytes, `14` source-manifest checks, and `0` issues.
...
- `python scripts/narrative_consistency.py --fail-on-error`: passed with `44` narrative checks and `0` issues.
- `python scripts/script_contracts.py --fail-on-error`: passed with `7` scripts, `101` contract checks, and `0` issues.
- `python scripts/claim_semantics.py --fail-on-error`: passed with `119` claims, `171` semantic checks, `51` CI-backed claims, and `0` issues.
- `python scripts/claim_evidence_quality.py --fail-on-error`: passed with `119` claims, `159` source links, `7` evidence checks, and `0` issues.
- `python scripts/tracked_artifact_provenance.py --fail-on-error`: passed with `103` claim sources, `433` artifact references, and `0` issues.
- `python scripts/claim_ledger_integrity.py --fail-on-error`: passed with `119` claims, `31` ledger checks, and `0` issues.
- `python scripts/claim_generation_consistency.py --fail-on-error`: passed with `119` claims, `10` generation checks, and `0` issues.
- `python scripts/report_generation_consistency.py --fail-on-error`: passed with `8` reports, `9` generation checks, and `0` issues.
- `python scripts/claims_status.py`: passed with `119` verified, `0` partial, `0` unsupported, `0` failed, and `0` README/paper overclaims.
