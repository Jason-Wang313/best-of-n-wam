...
- `python -m pytest -q`: passed with `151 passed`.
- `python scripts/test_inventory.py --fail-on-error`: passed with `151` collected tests, `6` inventory checks, and `0` issues.
...
- `python scripts/artifact_integrity.py --fail-on-error`: passed with `710` artifact references checked and `0` issues.
- `python scripts/artifact_manifest.py --fail-on-error`: passed with `412` scientific artifacts, `63117551` bytes, `15` manifest checks, and `0` issues.
...
- `python scripts/source_manifest.py --fail-on-error`: passed with `223` source files, `1604615` bytes, `14` source-manifest checks, and `0` issues.
...
- `python scripts/tracked_artifact_provenance.py --fail-on-error`: passed with `119` claim sources, `454` artifact references, and `0` issues.
- `python scripts/repo_bound_artifact_audit.py --fail-on-error`: passed with `885` records, `175` claim sources, `710` artifact references, `0` outside-repo records, `0` missing records, and `0` issues.
...
- `python scripts/evidence_hash_coverage.py --fail-on-error`: passed with `117` claim sources, `452` artifact references, `569` hashed records, and `0` issues.
