...
- `python -m pytest -q`: passed with `191 passed`.
- `python scripts/test_inventory.py --fail-on-error`: passed with `191` collected tests, `6` inventory checks, and `0` issues.
...
- `python scripts/artifact_integrity.py --fail-on-error`: passed with `729` artifact references checked and `0` issues.
- `python scripts/artifact_manifest.py --fail-on-error`: passed with `446` scientific artifacts, `77222641` bytes, `15` manifest checks, and `0` issues.
...
- `python scripts/source_manifest.py --fail-on-error`: passed with `246` source files, `1805886` bytes, `14` source-manifest checks, and `0` issues.
...
- `python scripts/repo_bound_artifact_audit.py --fail-on-error`: passed with `904` records, `175` claim sources, `729` artifact references, `0` outside-repo records, `0` missing records, and `0` issues.
