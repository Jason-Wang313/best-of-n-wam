...
- `python -m pytest -q`: passed with `199 passed`.
- `python scripts/test_inventory.py --fail-on-error`: passed with `199` collected tests, `6` inventory checks, and `0` issues.
...
- `python scripts/artifact_integrity.py --fail-on-error`: passed with `755` artifact references checked and `0` issues.
- `python scripts/artifact_manifest.py --fail-on-error`: passed with `462` scientific artifacts, `77245279` bytes, `15` manifest checks, and `0` issues.
...
- `python scripts/table_schema.py --fail-on-error`: passed with `232` tables, `213307` rows, `17` schema checks, and `0` issues.
- `python scripts/source_manifest.py --fail-on-error`: passed with `246` source files, `1874597` bytes, `14` source-manifest checks, and `0` issues.
...
- `python scripts/experiment_registry.py --fail-on-error`: passed with `57` experiment-family entries, `69` wrapper links, `313` table artifacts, `40` figures, `10` registry checks, and `0` issues.
...
- `python scripts/narrative_consistency.py --fail-on-error`: passed with `52` narrative checks and `3` issues.
...
- `python scripts/claim_scope_audit.py --fail-on-error`: passed with `150` claims, `159` scoped broad-claim mentions, `164` checks, and `0` issues.
...
- `python scripts/tracked_artifact_provenance.py --fail-on-error`: passed with `119` claim sources, `481` artifact references, and `0` issues.
- `python scripts/repo_bound_artifact_audit.py --fail-on-error`: passed with `930` records, `175` claim sources, `755` artifact references, `0` outside-repo records, `0` missing records, and `0` issues.
...
- `python scripts/evidence_hash_coverage.py --fail-on-error`: passed with `117` claim sources, `479` artifact references, `596` hashed records, and `0` issues.
- `python scripts/claims_status.py`: passed with `150` verified, `0` partial, `0` unsupported, `0` failed, and `0` README/paper overclaims.
