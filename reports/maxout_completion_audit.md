...
- RoboCasa registry coverage audit: registered task IDs `396`; rollout-pool task IDs `97`; micro-rollout task IDs `69`; any-artifact task IDs `97`.
...
- RoboCasa catalog coverage audit: local registry contains `396` task IDs; verified rollout-pool artifacts cover `97` of them, micro-rollout probes cover `69`, and any committed artifact covers `97`. This is coverage accounting, not validation for uncovered IDs.
...
- RoboCasa catalog coverage: `97` rollout-pool task IDs and `69` micro-rollout task IDs out of `396` registered task IDs.
