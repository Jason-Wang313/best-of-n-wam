...
- ManiSkill dependency probe: Pinocchio import `False`; robotics API `False`; external env Python count `3`; external env robotics API any `False`; binary PyPI `pinocchio` wheel `True`; binary `pin` wheel `False`; source install attempted `False`.
