...
- RoboCasa registry coverage audit: registered task IDs `396`; rollout-pool task IDs `28`; micro-rollout task IDs `4`; any-artifact task IDs `28`.
...
- RoboCasa catalog coverage audit: local registry contains `396` task IDs; verified rollout-pool artifacts cover `28` of them, micro-rollout probes cover `4`, and any committed artifact covers `28`. This is coverage accounting, not validation for uncovered IDs.
...
- RoboCasa catalog coverage: `28` rollout-pool task IDs and `4` micro-rollout task IDs out of `396` registered task IDs.
